"""
The four nodes of the Finance chatbot's LangGraph pipeline. Each node
...
def _entity(entities: dict, *keys: str) -> str | None:
    ...
    'transaction_id' instead of 'txn_reference') instead of the exact
    key name the retrieval SQL expects — the system prompt asks for
    exact key names, but a model picking its own synonym occasionally
    is cheap to tolerate here and expensive to silently get wrong (a
    transaction lookup falling through to an unfiltered query instead
    of erroring)."""
    for key in keys:
...
def retrieve_node(state: ChatState) -> dict:
    ...
    txn_reference = _entity(entities, "txn_reference", "transaction_id", "txn_id", "txnReference")

    if intent == "data_lookup":
        rows = repository.search_finance_transactions(
            dept_id,
            txn_reference=txn_reference,
            txn_type=entities.get("txn_type"),
            gl_account=entities.get("gl_account"),
            date_from=entities.get("date_from"),
            date_to=entities.get("date_to"),
            limit=config.MAX_FINANCE_ROWS,
        )
        for row in rows:
            context.append({
                "source": f"txn:{row['txn_reference']}",
                "type": "sql_row",
                "content": _format_txn_row(row),
            })

    elif intent == "dashboard_metric":
        rows = repository.aggregate_finance_by_type(dept_id, entities.get("date_from"), entities.get("date_to"))
        summary = "; ".join(
            f"{r['txn_type']}: {r['txn_count']} transactions, net ${r['net_amount'] or 0:,.2f}" for r in rows
        )
        context.append({
            "source": "finance_type_aggregate", "type": "sql_aggregate",
            "content": summary or "No finance transactions match those filters.",
        })
        gl_rows = repository.aggregate_finance_by_gl_account(dept_id)
        gl_summary = "; ".join(f"{r['gl_account']}/{r['txn_type']}: {r['txn_count']} txns, net ${r['net_amount'] or 0:,.2f}" for r in gl_rows[:20])
        if gl_summary:
            context.append({
                "source": "finance_gl_account_breakdown", "type": "sql_aggregate", "content": gl_summary,
            })
        ...
        if txn_reference:
            existing = repository.get_transaction_by_reference(dept_id, txn_reference)
            if existing:
                context.append({"source": f"txn:{existing['txn_reference']}", "type": "sql_row", "content": _format_txn_row(existing)})
        docs = repository.search_knowledge_docs(dept_id, "GL account mapping vendor payment approval accrual", limit=2)
        for doc in docs:
            context.append({"source": doc["title"], "type": f"knowledge_doc:{doc['doc_type']}", "content": doc["content"]})

    elif intent == "summarize":
        rows = repository.search_finance_transactions(
            dept_id, txn_type=entities.get("txn_type"), gl_account=entities.get("gl_account"),
            date_from=entities.get("date_from"), date_to=entities.get("date_to"), limit=config.MAX_FINANCE_ROWS,
        )
        for row in rows:
            context.append({"source": f"txn:{row['txn_reference']}", "type": "sql_row", "content": _format_txn_row(row)})
...
def hitl_draft_node(state: ChatState) -> dict:
    ...
    result = gateway_client.draft_hitl_record(
        ...
        retrieved_context=state.get("retrieved_context", []),
        allowed_fields=repository.FINANCE_COLUMNS,
        required_fields=repository.REQUIRED_FINANCE_FIELDS,
        conversation_history=state.get("conversation_history"),
    )
...
def _format_txn_row(row: dict) -> str:
