"""
The four nodes of the Prior Authorization chatbot's LangGraph pipeline.
...
def _entity(entities: dict, *keys: str) -> str | None:
    ...
    'pa_id' instead of 'pa_number') instead of the exact key name the
    retrieval SQL expects — the system prompt asks for exact key
    names, but a model picking its own synonym occasionally is cheap
    to tolerate here and expensive to silently get wrong (a PA lookup
    falling through to an unfiltered query instead of erroring)."""
    for key in keys:
...
def retrieve_node(state: ChatState) -> dict:
    ...
    pa_number = _entity(entities, "pa_number", "pa_id", "pa_no", "paNumber", "authorization_number")
    member_id = _entity(entities, "member_id", "memberId", "member_no")

    if intent == "data_lookup":
        rows = repository.search_prior_auths(
            dept_id,
            pa_number=pa_number,
            member_id=member_id,
            ...
            limit=config.MAX_PA_ROWS,
        )
        for row in rows:
            context.append({
                "source": f"prior_auth:{row['pa_number']}",
                "type": "sql_row",
                "content": _format_pa_row(row),
            })

    elif intent == "dashboard_metric":
        rows = repository.aggregate_pa_by_status(dept_id, entities.get("date_from"), entities.get("date_to"))
        summary = "; ".join(f"{r['status']}: {r['pa_count']} prior authorizations" for r in rows)
        context.append({
            "source": "prior_auth_status_aggregate", "type": "sql_aggregate",
            "content": summary or "No prior authorizations match those filters.",
        })
        urgency_rows = repository.aggregate_pa_by_urgency(dept_id)
        urgency_summary = "; ".join(f"{r['urgency']}/{r['status']}: {r['pa_count']}" for r in urgency_rows)
        if urgency_summary:
            context.append({
                "source": "prior_auth_urgency_breakdown", "type": "sql_aggregate", "content": urgency_summary,
            })
        ...
        if pa_number:
            existing = repository.get_pa_by_number(dept_id, pa_number)
            if existing:
                context.append({"source": f"prior_auth:{existing['pa_number']}", "type": "sql_row", "content": _format_pa_row(existing)})
        docs = repository.search_knowledge_docs(dept_id, "prior authorization medical necessity urgent escalation", limit=2)
        for doc in docs:
            context.append({"source": doc["title"], "type": f"knowledge_doc:{doc['doc_type']}", "content": doc["content"]})

    elif intent == "summarize":
        rows = repository.search_prior_auths(
            dept_id, member_id=member_id, status=entities.get("status"), urgency=entities.get("urgency"),
            date_from=entities.get("date_from"), date_to=entities.get("date_to"), limit=config.MAX_PA_ROWS,
        )
        for row in rows:
            context.append({"source": f"prior_auth:{row['pa_number']}", "type": "sql_row", "content": _format_pa_row(row)})
...
def hitl_draft_node(state: ChatState) -> dict:
    ...
    result = gateway_client.draft_hitl_record(
        session_id=state["session_id"],
        message=state["message"],
        entity_type="prior_authorizations",
        retrieved_context=state.get("retrieved_context", []),
        allowed_fields=repository.PA_COLUMNS,
        required_fields=repository.REQUIRED_PA_FIELDS,
        conversation_history=state.get("conversation_history"),
    )
...
def _format_pa_row(row: dict) -> str:
