"""
All SQL the Nursing chatbot needs, hand-written against the
health_ai_platform schema (owned by schema.sql — nothing here issues
DDL). Grouped by concern: identity, nursing case retrieval,
...
REQUIRED_NURSING_CASE_FIELDS = ["case_number", "member_id", "nurse_id", "case_type", "acuity_level", "status", "opened_date"]
NURSING_CASE_COLUMNS = [
    "case_number", "member_id", "nurse_id", "case_type", "acuity_level", "status",
    "opened_date", "closed_date", "care_plan_notes",
]
...
# --- Nursing case retrieval (for LangGraph's retrieve node) -------------

def search_nursing_cases(
    dept_id: int, *, case_number: str = None, member_id: str = None, status: str = None,
    case_type: str = None, acuity_level: str = None, date_from: str = None, date_to: str = None,
    limit: int = 25,
) -> list[dict]:
    clauses = ["dept_id = %s"]
    params: list = [dept_id]
    if case_number:
        clauses.append("case_number = %s")
    ...
    if acuity_level:
        clauses.append("acuity_level = %s")
        params.append(acuity_level.upper())
    if date_from:
        clauses.append("opened_date >= %s")
    ...
    sql = (
        "SELECT case_id, case_number, member_id, nurse_id, case_type, acuity_level, status, opened_date, "
        "closed_date, care_plan_notes FROM nursing_cases WHERE " + " AND ".join(clauses) +
        " ORDER BY opened_date DESC LIMIT %s"
    )
    params.append(limit)
...
def aggregate_cases_by_status(dept_id: int, date_from: str = None, date_to: str = None) -> list[dict]:
    clauses = ["dept_id = %s"]
    params: list = [dept_id]
    if date_from:
        clauses.append("opened_date >= %s")
    ...
    sql = (
        "SELECT status, COUNT(*) AS case_count FROM nursing_cases WHERE " + " AND ".join(clauses) +
...
def aggregate_cases_by_acuity(dept_id: int) -> list[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT acuity_level, status, COUNT(*) AS case_count FROM nursing_cases "
                "WHERE dept_id = %s GROUP BY acuity_level, status ORDER BY acuity_level, case_count DESC",
                (dept_id,),
            )
            return cur.fetchall()
    finally:
...
def get_case_by_number(dept_id: int, case_number: str) -> Optional[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM nursing_cases WHERE dept_id = %s AND case_number = %s", (dept_id, case_number)
            )
            return cur.fetchone()
    finally:
        conn.close()
...
_DATE_FIELDS = ["opened_date", "closed_date"]
...
def _clean_nursing_case_payload(payload: dict) -> dict:
    return {k: v for k, v in payload.items() if k in NURSING_CASE_COLUMNS}


def _validate_nursing_case_payload(payload: dict) -> None:
    missing = [f for f in REQUIRED_NURSING_CASE_FIELDS if not payload.get(f)]
    if missing:
...
def approve_hitl_task(task_id: int, reviewer_user_id: Optional[int], edited_payload: Optional[dict] = None,
    ...
    try:
        with conn.cursor() as cur:
            existing = None
            cur.execute(
                "SELECT case_id FROM nursing_cases WHERE dept_id = %s AND case_number = %s",
                (task["dept_id"], clean_payload["case_number"]),
            )
            existing = cur.fetchone()

            if existing:
                set_clause = ", ".join(f"{col} = %s" for col in clean_payload if col != "case_number")
                values = [v for k, v in clean_payload.items() if k != "case_number"]
                cur.execute(
                    f"UPDATE nursing_cases SET {set_clause} WHERE case_id = %s",
                    values + [existing["case_id"]],
                )
                entity_ref_id = existing["case_id"]
            else:
                cols = ["dept_id"] + list(clean_payload.keys())
                placeholders = ["%s"] * len(cols)
                cur.execute(
                    f"INSERT INTO nursing_cases ({', '.join(cols)}) VALUES ({', '.join(placeholders)})",
