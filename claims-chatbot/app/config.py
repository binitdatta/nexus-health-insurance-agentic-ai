"""
Configuration for the Claims department chatbot. Every department
chatbot in the platform is a near-identical copy of this app with
different DEPT_CODE / KEYCLOAK_CLIENT_ID / branding — see the README
for what to change to stand up the next one.
"""
import os
...
class Config:
    ...
    DEPT_CODE = os.getenv("DEPT_CODE", "CLAIMS")
    CHATBOT_SOURCE = os.getenv("CHATBOT_SOURCE", "claims-chatbot")
    DEPT_DISPLAY_NAME = os.getenv("DEPT_DISPLAY_NAME", "Claims")
    ...
    KEYCLOAK_CLIENT_ID = os.getenv("KEYCLOAK_CLIENT_ID", "claims-chatbot-pkce")
    ...
    OAUTH_REDIRECT_URI = os.getenv("OAUTH_REDIRECT_URI", "http://localhost:5001/auth/callback")
    OAUTH_STATE_MAX_AGE_SECONDS = int(os.getenv("OAUTH_STATE_MAX_AGE_SECONDS", "300"))
    POST_LOGOUT_REDIRECT_URI = os.getenv("POST_LOGOUT_REDIRECT_URI", "http://localhost:5001/")
    ...
    LOG_FILE_NAME = os.getenv("LOG_FILE_NAME", "claims_chatbot.log")
    LOG_MAX_BYTES = int(os.getenv("LOG_MAX_BYTES", str(25 * 1024 * 1024)))
    LOG_BACKUP_COUNT = int(os.getenv("LOG_BACKUP_COUNT", "10"))
    LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO")

    # --- Retrieval tuning -----------------------------------------------
    MAX_CLAIMS_ROWS = int(os.getenv("MAX_CLAIMS_ROWS", "25"))
