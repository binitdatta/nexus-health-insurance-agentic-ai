"""
The four nodes of the Claims chatbot's LangGraph pipeline. Each node
...
def _entity(entities: dict, *keys: str) -> str | None:
    ...
    'claim_id') instead of the exact key name the retrieval SQL expects
    — the system prompt now asks for exact key names, but a model
    picking its own synonym occasionally is cheap to tolerate here and
    expensive to silently get wrong (a claim lookup falling through to
    an unfiltered query instead of erroring)."""
    for key in keys:
...
def retrieve_node(state: ChatState) -> dict:
    ...
    if intent == "data_lookup":
        rows = repository.search_claims(
            ...
            limit=config.MAX_CLAIMS_ROWS,
        )
        for row in rows:
            context.append({
                "source": f"claims:{row['claim_number']}",
                "type": "sql_row",
                "content": _format_claim_row(row),
            })

    elif intent == "dashboard_metric":
        rows = repository.aggregate_claims_by_status(dept_id, entities.get("date_from"), entities.get("date_to"))
        summary = "; ".join(
            f"{r['claim_status']}: {r['claim_count']} claims, billed ${r['total_billed'] or 0:,.2f}, "
            f"paid ${r['total_paid'] or 0:,.2f}" for r in rows
        )
        context.append({"source": "claims_status_aggregate", "type": "sql_aggregate", "content": summary or "No claims match those filters."})
        ...
        claim_number = _entity(entities, "claim_number", "claim_id", "claim_no", "claimNumber")
        if claim_number:
            existing = repository.get_claim_by_number(dept_id, claim_number)
            if existing:
                context.append({"source": f"claims:{existing['claim_number']}", "type": "sql_row", "content": _format_claim_row(existing)})
        docs = repository.search_knowledge_docs(dept_id, "claims appeals denial", limit=2)
        for doc in docs:
            context.append({"source": doc["title"], "type": f"knowledge_doc:{doc['doc_type']}", "content": doc["content"]})

    elif intent == "summarize":
        rows = repository.search_claims(
            dept_id, member_id=_entity(entities, "member_id", "memberId", "member_no"), status=entities.get("status"),
            date_from=entities.get("date_from"), date_to=entities.get("date_to"), limit=config.MAX_CLAIMS_ROWS,
        )
        for row in rows:
            context.append({"source": f"claims:{row['claim_number']}", "type": "sql_row", "content": _format_claim_row(row)})
...
def hitl_draft_node(state: ChatState) -> dict:
    ...
    result = gateway_client.draft_hitl_record(
        ...
        entity_type="claims",
        retrieved_context=state.get("retrieved_context", []),
        allowed_fields=repository.CLAIM_COLUMNS,
        required_fields=repository.REQUIRED_CLAIM_FIELDS,
        conversation_history=state.get("conversation_history"),
    )
...
def _format_claim_row(row: dict) -> str:
