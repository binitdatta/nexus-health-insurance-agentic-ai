...
def test_dashboard_totals_reflect_real_seed_data(app):
    with app.app_context():
        ...
        # 'billing-chatbot' (see generate_seed_data.py) — confirm the
        # dashboard's own aggregate query actually sees them.
        totals = repository.dashboard_totals(dept_id, "billing-chatbot")
...
def test_dashboard_daily_summary_ordered_ascending(app):
    with app.app_context():
        dept_id = repository.get_dept_id("BILLING")
        daily = repository.dashboard_daily_summary(dept_id, "billing-chatbot", days=14)
