"""
All SQL the Billing chatbot needs, hand-written against the
health_ai_platform schema (owned by schema.sql — nothing here issues
DDL). Grouped by concern: identity, billing record retrieval,
...
REQUIRED_BILLING_FIELDS = ["invoice_number", "member_id", "billing_period", "amount_due", "payment_status", "due_date"]
BILLING_COLUMNS = [
    "invoice_number", "member_id", "billing_period", "amount_due", "amount_paid", "payment_status",
...
# --- Billing record retrieval (for LangGraph's retrieve node) -------------

def search_billing_records(
    dept_id: int, *, invoice_number: str = None, member_id: str = None, payment_status: str = None,
    billing_period: str = None, date_from: str = None, date_to: str = None, limit: int = 25,
) -> list[dict]:
    clauses = ["dept_id = %s"]
    params: list = [dept_id]
    if invoice_number:
    ...
    if billing_period:
        clauses.append("billing_period = %s")
    ...
    sql = (
        "SELECT billing_id, invoice_number, member_id, billing_period, amount_due, amount_paid, "
        "payment_status, due_date, paid_date, payment_method FROM billing_records WHERE " +
        " AND ".join(clauses) + " ORDER BY due_date DESC LIMIT %s"
    )
    params.append(limit)
...
def aggregate_billing_by_status(dept_id: int, date_from: str = None, date_to: str = None) -> list[dict]:
    clauses = ["dept_id = %s"]
    params: list = [dept_id]
    if date_from:
        clauses.append("due_date >= %s")
    ...
    sql = (
        "SELECT payment_status, COUNT(*) AS invoice_count, SUM(amount_due) AS total_due, "
        "SUM(amount_paid) AS total_paid FROM billing_records WHERE " + " AND ".join(clauses) +
        " GROUP BY payment_status ORDER BY invoice_count DESC"
    )
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
...
def aggregate_billing_by_method(dept_id: int) -> list[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT COALESCE(payment_method, 'UNPAID_NO_METHOD') AS payment_method, payment_status, "
                "COUNT(*) AS invoice_count FROM billing_records "
                "WHERE dept_id = %s GROUP BY payment_method, payment_status ORDER BY payment_method, invoice_count DESC",
                (dept_id,),
            )
            return cur.fetchall()
    finally:
...
def get_invoice_by_number(dept_id: int, invoice_number: str) -> Optional[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM billing_records WHERE dept_id = %s AND invoice_number = %s", (dept_id, invoice_number)
            )
            return cur.fetchone()
    finally:
        conn.close()
...
# Real column names in billing_records that are MySQL DATE columns (not
# DATETIME — see Call Center's chatbot for that variant). A reviewer
# typing a normal date is common and reasonable; MySQL's DATE type only
# accepts 'YYYY-MM-DD' and will otherwise fail with an opaque SQL error
# (pymysql.err.OperationalError 1292) that reaches the reviewer as a raw
# 500 if not caught first — this normalizes common formats BEFORE the
# value ever reaches the database, so a real human date actually works
# instead of just failing with a nicer message.
_DATE_FIELDS = ["due_date", "paid_date"]

# Tried in order; the first one that parses wins. Deliberately excludes
# ambiguous day-first formats (e.g. '%d/%m/%Y') since '3/4/2026' means a
# different date in US vs. international convention and silently
# guessing wrong is worse than requiring an unambiguous format.
...
def _clean_billing_payload(payload: dict) -> dict:
    return {k: v for k, v in payload.items() if k in BILLING_COLUMNS}


def _validate_billing_payload(payload: dict) -> None:
    missing = [f for f in REQUIRED_BILLING_FIELDS if not payload.get(f)]
    if missing:
...
def approve_hitl_task(task_id: int, reviewer_user_id: Optional[int], edited_payload: Optional[dict] = None,
    ...
    try:
        with conn.cursor() as cur:
            existing = None
            cur.execute(
                "SELECT billing_id FROM billing_records WHERE dept_id = %s AND invoice_number = %s",
                (task["dept_id"], clean_payload["invoice_number"]),
            )
            existing = cur.fetchone()

            if existing:
                set_clause = ", ".join(f"{col} = %s" for col in clean_payload if col != "invoice_number")
                values = [v for k, v in clean_payload.items() if k != "invoice_number"]
                cur.execute(
                    f"UPDATE billing_records SET {set_clause} WHERE billing_id = %s",
                    values + [existing["billing_id"]],
                )
                entity_ref_id = existing["billing_id"]
            else:
                cols = ["dept_id"] + list(clean_payload.keys())
                placeholders = ["%s"] * len(cols)
                cur.execute(
                    f"INSERT INTO billing_records ({', '.join(cols)}) VALUES ({', '.join(placeholders)})",
