"""
All SQL the Facility & Providers chatbot needs, hand-written against
the health_ai_platform schema (owned by schema.sql — nothing here
issues DDL). Grouped by concern: identity, provider directory
...
REQUIRED_PROVIDER_FIELDS = ["provider_code", "provider_name", "npi_number", "network_status"]
PROVIDER_COLUMNS = [
    "provider_code", "provider_name", "npi_number", "specialty", "facility_name", "network_status",
...
# --- Provider directory retrieval (for LangGraph's retrieve node) --------
# Note: unlike every other department chatbot in this platform, `providers`
# has no member_id column — this is directory data about providers/
# facilities themselves, not member-linked transactional data. Retrieval
# keys on provider_code, npi_number, and provider_name instead.

def search_providers(
    dept_id: int, *, provider_code: str = None, npi_number: str = None, provider_name: str = None,
    specialty: str = None, network_status: str = None, limit: int = 25,
) -> list[dict]:
    clauses = ["dept_id = %s"]
    params: list = [dept_id]
    if provider_code:
    ...
    if npi_number:
        clauses.append("npi_number = %s")
        params.append(npi_number)
    if provider_name:
        clauses.append("provider_name LIKE %s")
        params.append(f"%{provider_name}%")
    if specialty:
        clauses.append("specialty = %s")
        params.append(specialty)
    if network_status:
        clauses.append("network_status = %s")
        params.append(network_status.upper())

    sql = (
        "SELECT provider_id, provider_code, provider_name, npi_number, specialty, facility_name, "
        "network_status, address, phone, contract_start, contract_end FROM providers WHERE " +
        " AND ".join(clauses) + " ORDER BY provider_name ASC LIMIT %s"
    )
    params.append(limit)
...
def aggregate_providers_by_status(dept_id: int) -> list[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT network_status, COUNT(*) AS provider_count FROM providers "
                "WHERE dept_id = %s GROUP BY network_status ORDER BY provider_count DESC",
                (dept_id,),
            )
            return cur.fetchall()
    finally:
...
def aggregate_providers_by_specialty(dept_id: int) -> list[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT specialty, network_status, COUNT(*) AS provider_count FROM providers "
                "WHERE dept_id = %s GROUP BY specialty, network_status ORDER BY specialty, provider_count DESC",
                (dept_id,),
            )
            return cur.fetchall()
    finally:
...
def get_provider_by_code(dept_id: int, provider_code: str) -> Optional[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM providers WHERE dept_id = %s AND provider_code = %s", (dept_id, provider_code)
            )
            return cur.fetchone()
    finally:
        conn.close()
...
_DATE_FIELDS = ["contract_start", "contract_end"]
...
def _clean_provider_payload(payload: dict) -> dict:
    return {k: v for k, v in payload.items() if k in PROVIDER_COLUMNS}


def _validate_provider_payload(payload: dict) -> None:
    missing = [f for f in REQUIRED_PROVIDER_FIELDS if not payload.get(f)]
    if missing:
...
def approve_hitl_task(task_id: int, reviewer_user_id: Optional[int], edited_payload: Optional[dict] = None,
    ...
    try:
        with conn.cursor() as cur:
            existing = None
            cur.execute(
                "SELECT provider_id FROM providers WHERE dept_id = %s AND provider_code = %s",
                (task["dept_id"], clean_payload["provider_code"]),
            )
            existing = cur.fetchone()

            if existing:
                set_clause = ", ".join(f"{col} = %s" for col in clean_payload if col != "provider_code")
                values = [v for k, v in clean_payload.items() if k != "provider_code"]
                cur.execute(
                    f"UPDATE providers SET {set_clause} WHERE provider_id = %s",
                    values + [existing["provider_id"]],
                )
                entity_ref_id = existing["provider_id"]
            else:
                cols = ["dept_id"] + list(clean_payload.keys())
                placeholders = ["%s"] * len(cols)
                cur.execute(
                    f"INSERT INTO providers ({', '.join(cols)}) VALUES ({', '.join(placeholders)})",
