"""
The four nodes of the Call Center chatbot's LangGraph pipeline. Each
...
def _entity(entities: dict, *keys: str) -> str | None:
    ...
    'call_id' instead of 'call_reference') instead of the exact key
    ...
    cheap to tolerate here and expensive to silently get wrong (a call
...
def retrieve_node(state: ChatState) -> dict:
    ...
    call_reference = _entity(entities, "call_reference", "call_id", "callReference")
    member_id = _entity(entities, "member_id", "memberId", "member_no")
    agent_id = _entity(entities, "agent_id", "agentId")

    if intent == "data_lookup":
        rows = repository.search_calls(
            dept_id,
            call_reference=call_reference,
            member_id=member_id,
            agent_id=agent_id,
            call_type=entities.get("call_type"),
            resolution_status=entities.get("resolution_status") or entities.get("status"),
            date_from=entities.get("date_from"),
            date_to=entities.get("date_to"),
            limit=config.MAX_CALL_ROWS,
        )
        for row in rows:
            context.append({
                "source": f"call:{row['call_reference']}",
                "type": "sql_row",
                "content": _format_call_row(row),
            })

    elif intent == "dashboard_metric":
        rows = repository.aggregate_calls_by_type(dept_id, entities.get("date_from"), entities.get("date_to"))
        summary = "; ".join(
            f"{r['call_type']}: {r['call_count']} calls, avg CSAT {r['avg_csat']}" for r in rows
        )
        context.append({
            "source": "call_type_aggregate", "type": "sql_aggregate",
            "content": summary or "No calls match those filters.",
        })
        resolution_rows = repository.aggregate_calls_by_resolution(dept_id)
        resolution_summary = "; ".join(
            f"{r['resolution_status']}/{r['call_type']}: {r['call_count']} calls, avg CSAT {r['avg_csat']}"
            for r in resolution_rows
        )
        if resolution_summary:
            context.append({
                "source": "call_resolution_breakdown", "type": "sql_aggregate", "content": resolution_summary,
            })
        ...
        if call_reference:
            existing = repository.get_call_by_reference(dept_id, call_reference)
            if existing:
                context.append({"source": f"call:{existing['call_reference']}", "type": "sql_row", "content": _format_call_row(existing)})
        docs = repository.search_knowledge_docs(dept_id, "escalation grievance CSAT survey call handling", limit=2)
        for doc in docs:
            context.append({"source": doc["title"], "type": f"knowledge_doc:{doc['doc_type']}", "content": doc["content"]})

    elif intent == "summarize":
        rows = repository.search_calls(
            dept_id, member_id=member_id, agent_id=agent_id, call_type=entities.get("call_type"),
            resolution_status=entities.get("resolution_status") or entities.get("status"),
            date_from=entities.get("date_from"), date_to=entities.get("date_to"), limit=config.MAX_CALL_ROWS,
        )
        for row in rows:
            context.append({"source": f"call:{row['call_reference']}", "type": "sql_row", "content": _format_call_row(row)})
...
def hitl_draft_node(state: ChatState) -> dict:
    ...
    result = gateway_client.draft_hitl_record(
        session_id=state["session_id"],
        message=state["message"],
        entity_type="call_center_logs",
        retrieved_context=state.get("retrieved_context", []),
        allowed_fields=repository.CALL_COLUMNS,
        required_fields=repository.REQUIRED_CALL_FIELDS,
        conversation_history=state.get("conversation_history"),
    )
