...
def test_data_lookup_retrieves_real_adjudication_and_renders_table(app, logged_in_client, canned_gateway_responses):
    canned_gateway_responses["intent"] = {
        "intent": "data_lookup",
        "confidence": 0.95,
        "entities": {"claim_number": "CLM100000"},
    ...
    def fake_respond(payload):
        ...
        return {
            "answer_markdown": "CLM100000's adjudication details below.",
            "render": {"type": "table", "spec": {"columns": ["claim_number"], "rows": [["CLM100000"]]}},
            "citations": ["adjudication:1:CLM100000"],
            "confidence": 0.9,
    ...
    with app.test_request_context("/api/chat"):
        ...
        session["user"] = {"sub": "dev-user-sub", "username": "dev.tester", "email": "e@x.com", "department": "ADJUDICATION", "roles": []}
        session["chat_session_id"] = "test-session-1"

        final_state = run_chat_turn(session_id="test-session-1", message="How was claim CLM100000 adjudicated?")

    assert final_state["intent"] == "data_lookup"
    # Retrieval must have actually queried the real seeded adjudication_records table
    assert len(captured_context["retrieved_context"]) == 1
    assert captured_context["retrieved_context"][0]["source"] == "adjudication:1:CLM100000"
    assert "claim_number=CLM100000" in captured_context["retrieved_context"][0]["content"]
    assert "ADJ145" in captured_context["retrieved_context"][0]["content"]  # real seeded adjudicator for this row
    assert "Bundling Edit" in captured_context["retrieved_context"][0]["content"]
    assert final_state["render"]["type"] == "table"
    assert final_state.get("hitl") is None  # no HITL for a lookup


def test_data_lookup_tolerates_claim_id_synonym_from_llm(app, canned_gateway_responses):
    ...
    entity 'claim_id' instead of 'claim_number'. retrieve_node must
    not silently fall through to an unfiltered query when that
    happens."""
    canned_gateway_responses["intent"] = {
        "intent": "data_lookup", "confidence": 0.9,
        "entities": {"claim_id": "CLM100000"},  # deliberately the "wrong" key
        "suggested_render": "table",
    ...
    with app.test_request_context("/api/chat"):
        from flask import session
        session["user"] = {"sub": "dev-user-sub", "username": "dev.tester", "email": "e@x.com", "department": "ADJUDICATION", "roles": []}
        run_chat_turn(session_id="test-session-2", message="How was claim CLM100000 adjudicated?")

    assert len(captured["ctx"]) == 1
    assert captured["ctx"][0]["source"] == "adjudication:1:CLM100000"
...
def test_policy_question_retrieves_knowledge_docs(app, canned_gateway_responses):
    ...
    with app.test_request_context("/api/chat"):
        from flask import session
        session["user"] = {"sub": "dev-user-sub", "username": "dev.tester", "email": "e@x.com", "department": "ADJUDICATION", "roles": []}
        final_state = run_chat_turn(session_id="test-session-3", message="How does our bundling edit rule work?")
...
def test_create_record_intent_triggers_hitl_draft_and_inserts_task(app, canned_gateway_responses):
    canned_gateway_responses["intent"] = {
        ...
        "entities": {"claim_number": "CLM100050", "rule_applied": "Duplicate Check"},
    ...
    canned_gateway_responses["respond"] = {
        "answer_markdown": "I've drafted an adjudication decision for you to review.",
    ...
    canned_gateway_responses["hitl-draft"] = {
        "proposed_payload": {"claim_number": "CLM100050", "rule_applied": "Duplicate Check", "decision": "DENY",
                              "adjudicated_date": "2026-01-01", "notes": "TEST_ROW_SAFE_TO_DELETE"},
        "missing_fields": ["adjudicator_id"],
        "rationale": "User asked to adjudicate this claim under the duplicate check rule.",
    ...
    with app.test_request_context("/api/chat"):
        from flask import session
        session["user"] = {"sub": "dev-user-sub", "username": "dev.tester", "email": "e@x.com", "department": "ADJUDICATION", "roles": []}
        final_state = run_chat_turn(session_id="test-session-4", message="Adjudicate claim CLM100050 as a duplicate")

    assert final_state["intent"] == "create_record"
    assert final_state["hitl"] is not None
    assert final_state["hitl"]["task_id"] > 0
    assert final_state["hitl"]["proposed_payload"]["decision"] == "DENY"
    assert "adjudicator_id" in final_state["hitl"]["missing_fields"]

    from app import repository
    task = repository.get_hitl_task(final_state["hitl"]["task_id"])
    assert task["status"] == "PENDING"
    assert task["entity_type"] == "adjudication_records"
...
def test_hitl_draft_sends_real_schema_not_invented_fields(app, canned_gateway_responses):
    ...
    adjudication_records columns, never invented field names with
    nowhere to be saved."""
    canned_gateway_responses["intent"] = {
        "intent": "create_record", "confidence": 0.9,
        "entities": {"claim_number": "CLM100000"}, "suggested_render": "none",
    ...
    def fake_hitl(payload):
        ...
        return {
            "proposed_payload": {"claim_number": "CLM100000", "decision": "ADJUST"},
            "missing_fields": ["adjudicator_id", "rule_applied", "adjudicated_date"],
            "rationale": "test",
    ...
    with app.test_request_context("/api/chat"):
        from flask import session
        session["user"] = {"sub": "dev-user-sub", "username": "dev.tester", "email": "e@x.com", "department": "ADJUDICATION", "roles": []}
        run_chat_turn(session_id="test-session-5", message="Re-adjudicate claim CLM100000")

    from app import repository
    assert captured["entity_type"] == "adjudication_records"
    assert captured["allowed_fields"] == repository.ADJUDICATION_COLUMNS
    assert captured["required_fields"] == repository.REQUIRED_ADJUDICATION_FIELDS
    for invented in ("appeal_status", "auditor_review_notes", "escalation_flag"):
...
def test_usage_events_accumulate_across_nodes(app, canned_gateway_responses):
    ...
    with app.test_request_context("/api/chat"):
        from flask import session
        session["user"] = {"sub": "dev-user-sub", "username": "dev.tester", "email": "e@x.com", "department": "ADJUDICATION", "roles": []}
        final_state = run_chat_turn(session_id="test-session-6", message="How many claims were approved vs denied?")
