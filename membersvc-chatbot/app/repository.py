"""
All SQL the Member Services chatbot needs, hand-written against the
health_ai_platform schema (owned by schema.sql — nothing here issues
DDL). Grouped by concern: identity, ticket retrieval, knowledge_docs
...
REQUIRED_TICKET_FIELDS = ["ticket_number", "member_id", "agent_id", "category", "priority", "status", "opened_date"]
TICKET_COLUMNS = [
    "ticket_number", "member_id", "agent_id", "category", "priority", "status",
    "opened_date", "closed_date", "resolution_notes",
]
...
# --- Ticket retrieval (for LangGraph's retrieve node) --------------------

def search_tickets(
    dept_id: int, *, ticket_number: str = None, member_id: str = None, status: str = None,
    category: str = None, priority: str = None, date_from: str = None, date_to: str = None,
    limit: int = 25,
) -> list[dict]:
    clauses = ["dept_id = %s"]
    params: list = [dept_id]
    if ticket_number:
        clauses.append("ticket_number = %s")
    ...
    sql = (
        "SELECT ticket_id, ticket_number, member_id, agent_id, category, priority, status, opened_date, "
        "closed_date, resolution_notes FROM member_services_tickets WHERE " + " AND ".join(clauses) +
...
def aggregate_tickets_by_status(dept_id: int, date_from: str = None, date_to: str = None) -> list[dict]:
    clauses = ["dept_id = %s"]
    params: list = [dept_id]
    if date_from:
        clauses.append("opened_date >= %s")
    ...
    sql = (
        "SELECT status, COUNT(*) AS ticket_count FROM member_services_tickets WHERE " + " AND ".join(clauses) +
...
def aggregate_tickets_by_category(dept_id: int) -> list[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT category, priority, COUNT(*) AS ticket_count FROM member_services_tickets "
                "WHERE dept_id = %s GROUP BY category, priority ORDER BY category, ticket_count DESC",
                (dept_id,),
            )
            return cur.fetchall()
    finally:
...
def get_ticket_by_number(dept_id: int, ticket_number: str) -> Optional[dict]:
    conn = extensions.get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM member_services_tickets WHERE dept_id = %s AND ticket_number = %s",
                (dept_id, ticket_number),
            )
            return cur.fetchone()
    finally:
        conn.close()
...
def _clean_ticket_payload(payload: dict) -> dict:
    return {k: v for k, v in payload.items() if k in TICKET_COLUMNS}


def _validate_ticket_payload(payload: dict) -> None:
    missing = [f for f in REQUIRED_TICKET_FIELDS if not payload.get(f)]
    if missing:
...
def approve_hitl_task(task_id: int, reviewer_user_id: Optional[int], edited_payload: Optional[dict] = None,
    ...
    try:
        with conn.cursor() as cur:
            existing = None
            cur.execute(
                "SELECT ticket_id FROM member_services_tickets WHERE dept_id = %s AND ticket_number = %s",
                (task["dept_id"], clean_payload["ticket_number"]),
            )
            existing = cur.fetchone()

            if existing:
                set_clause = ", ".join(f"{col} = %s" for col in clean_payload if col != "ticket_number")
                values = [v for k, v in clean_payload.items() if k != "ticket_number"]
                cur.execute(
                    f"UPDATE member_services_tickets SET {set_clause} WHERE ticket_id = %s",
                    values + [existing["ticket_id"]],
                )
                entity_ref_id = existing["ticket_id"]
            else:
                cols = ["dept_id"] + list(clean_payload.keys())
                placeholders = ["%s"] * len(cols)
                cur.execute(
                    f"INSERT INTO member_services_tickets ({', '.join(cols)}) VALUES ({', '.join(placeholders)})",
